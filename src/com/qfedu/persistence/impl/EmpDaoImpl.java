...
public class EmpDaoImpl implements EmpDao {
	...
	private static final String UPDATE_EMP_SQL = 
			"update tb_emp set ename=?, ejob=?, emgr=?, esal=?, estatus=?, etel=?, dno=? where eno=?";
	...
	public PageBean<EmpDto> findEmpsByDeptNo(Integer no, int page, int size) {
		...
		ResultSet rs2 = DbSessionFactory.getCurrentDbSession().executeQuery(
	...
	public boolean update(Emp emp) {
		return DbSessionFactory.getCurrentDbSession().executeUpdate(
				UPDATE_EMP_SQL,
				emp.getName(), emp.getJob(), emp.getMgr().getNo(),
				emp.getSalary(), emp.getStatus(), emp.getTel(),
				emp.getDept().getNo(), emp.getNo()) == 1;
	}
